...
| `MATOMO_EXCLUDE_DATA_FROM_PERSIST`       | Files to exclude from being persisted relative to the Matomo installation directory (same as config.ini.php). To provide multiple values, separate them with a whitespace. No defaults.                                                 | `nil`                           |
