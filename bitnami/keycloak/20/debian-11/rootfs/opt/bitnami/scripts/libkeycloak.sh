...
keycloak_configure_database() {
    ...
    if [ "${KEYCLOAK_DATABASE_VENDOR}" == "oracle" ]; then
      keycloak_conf_set "db" "oracle"
      keycloak_conf_set "db-url" "jdbc:oracle:thin:@${KEYCLOAK_DATABASE_HOST}:${KEYCLOAK_DATABASE_PORT}:${KEYCLOAK_DATABASE_NAME}?currentSchema=${KEYCLOAK_DATABASE_SCHEMA}${jdbc_params}"
    else
