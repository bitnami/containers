...
#   None
#########################
scylla_get_minor_version() {
    scylla_version="$("${DB_BIN_DIR}/scylla" --version)"
    minor_version="$(get_sematic_version "$scylla_version" 2)"
    echo "${minor_version:-0}"
}

########################
# Checks whether the running scylla release supports pre-seeding the default
# superuser via the auth_superuser_name/auth_superuser_salted_password config
# keys instead of relying on the built-in cassandra/cassandra account.
# ScyllaDB 2026.2 removed the default cassandra/cassandra superuser
# (https://github.com/scylladb/scylladb/issues/27215), so on 2026.2+ the
# superuser must be seeded via config before the first boot.
# Globals:
#   DB_FLAVOR
# Arguments:
#   None
...
#   Boolean
#########################
scylladb_supports_auth_superuser_config() {
    [[ "$DB_FLAVOR" = "scylladb" ]] || return 1
    local -r major="$(scylla_get_major_version)"
    local -r minor="$(scylla_get_minor_version)"
    [[ "$major" -gt 2026 ]] || { [[ "$major" -eq 2026 ]] && [[ "$minor" -ge 2 ]]; }
}

########################
# Computes a salted password hash suitable for the auth_superuser_salted_password
