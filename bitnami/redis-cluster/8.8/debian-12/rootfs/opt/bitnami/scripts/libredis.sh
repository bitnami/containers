...
redis_configure_replication() {
    ...
    if [[ "$REDIS_REPLICATION_MODE" = "master" ]]; then
        ...
        if [[ -n "$REDIS_SENTINEL_HOST" ]]; then
            ...
            if is_boolean_yes "$REDIS_TLS_ENABLED"; then
                sentinel_info_command+=("--tls" "--cert" "${REDIS_TLS_CERT_FILE}" "--key" "${REDIS_TLS_KEY_FILE}")
            ...
            read -r -a REDIS_SENTINEL_INFO <<< "$(REDISCLI_AUTH="${REDIS_SENTINEL_PASSWORD:-}" "${sentinel_info_command[@]}" | tr '\n' ' ')"
