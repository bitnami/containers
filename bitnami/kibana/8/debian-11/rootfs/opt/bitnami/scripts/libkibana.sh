...
kibana_initialize() {
    ...
    if is_file_writable "$SERVER_CONF_FILE"; then
        ...
        if ! is_empty_value "$SERVER_PUBLICBASEURL"; then
            kibana_conf_set "server.publicBaseUrl" "$SERVER_PUBLICBASEURL"
        fi
        ...
        if ! is_empty_value "$XPACK_SECURITY_ENCRYPTIONKEY"; then
            kibana_conf_set "xpack.security.encryptionKey" "$XPACK_SECURITY_ENCRYPTIONKEY"
        fi
        ...
        if ! is_empty_value "$XPACK_REPORTING_ENCRYPTIONKEY"; then
            kibana_conf_set "xpack.reporting.encryptionKey" "$XPACK_REPORTING_ENCRYPTIONKEY"
        fi

        #Controls whether to enable the newsfeed system for the Kibana UI notification center
        if ! is_boolean_yes "$NEWSFEED_ENABLED"; then
        ...
        if [[ "$ELASTICSEARCH_REQUESTTIMEOUT" != "30000" ]]; then
            kibana_conf_set "elasticsearch.requestTimeout" "$ELASTICSEARCH_REQUESTTIMEOUT"
        fi                              
