...
if [[ ! -f "/bitnami/phpmyadmin/.user_scripts_initialized" && -d "/docker-entrypoint-init.d" ]]; then
    read -r -a init_scripts <<< "$(find "/docker-entrypoint-init.d" -type f -print0 | sort -z | xargs -0)"
    if [[ "${#init_scripts[@]}" -gt 0 ]] && [[ ! -f "/bitnami/phpmyadmin/.user_scripts_initialized" ]]; then
        mkdir -p "/bitnami/phpmyadmin"
