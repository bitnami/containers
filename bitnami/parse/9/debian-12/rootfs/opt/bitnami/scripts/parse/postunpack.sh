...
find "${PARSE_BASE_DIR}/node_modules" -name ".bin" -type d -exec rm -rf {} +
...
for dir in "$PARSE_VOLUME_DIR" "$PARSE_LOGS_DIR" "$PARSE_TMP_DIR"; do
    ensure_dir_exists "$dir"
    configure_permissions_ownership "$dir" -d "775" -u "$PARSE_DAEMON_USER" -g "$PARSE_DAEMON_GROUP"
done
# Use daemon:root ownership for compatibility when running as a non-root user
configure_permissions_ownership "$PARSE_BASE_DIR" -d "775" -f "664" -u "$PARSE_DAEMON_USER" -g "root"
